...
static int init_b_cas_card(void *bcas)
{
	...
	m = 0x10 + (2*B_CAS_BUFFER_MAX) + (sizeof(int64_t)*16) + (sizeof(B_CAS_PWR_ON_CTRL)*16);
...
static int proc_ecm_b_cas_card(void *bcas, B_CAS_ECM_RESULT *dst, uint8_t *src, int len)
{
	...
	retry_count = 0;
	ret = KtvCardTransmit(prv->carddev, prv->sbuf, slen, prv->rbuf, &rlen);
	while( ((ret != 0) || (rlen < 25)) && (retry_count < 10) ){
		retry_count += 1;
		if(!connect_card(prv, prv->reader)){
			continue;
		}
...
static int proc_emm_b_cas_card(void *bcas, uint8_t *src, int len)
{
	...
	retry_count = 0;
	ret = KtvCardTransmit(prv->carddev, prv->sbuf, slen, prv->rbuf, &rlen);
	while( ((ret != 0) || (rlen < 6)) && (retry_count < 2) ){
		retry_count += 1;
		if(!connect_card(prv, prv->reader)){
			continue;
		}
...
static int connect_card(B_CAS_CARD_PRIVATE_DATA *prv, const char *reader_name)
{
	...
	rlen = B_CAS_BUFFER_MAX;
	ret = KtvCardTransmit(prv->carddev, prv->sbuf, m, prv->rbuf, &rlen);
	if(ret != 0){
		return 0;
	}
